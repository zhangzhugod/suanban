<template>
  <div class="event">
    ...
    <header>
      ...
      <select v-model="selDate" @change="changeDate" id="">
      ...
      <select v-model="selType" @change="changeType" id="">
      ...
      <select v-model="selFee" @change="changeFree" id="">
    ...
    <div class="m-wrap">
      <div v-for="event in arr">
        <a :href="event.url">
          ...
          <div class="right">
            <span class="m-title">{{event.title}}{{'【'+event.category_name+'】'}}</span><br/>
            ...
            <span class="rt"><strong>费用：</strong><span :class="{'free-event':selFee === 'all' && event.fee_str ==='免费'}">{{event.fee_str}}</span></span><br/>
  ...
  export default {
    ...
    data() {
      return {
        ...
        cityList:[],
        selFee: localStorage.freeType || 'free',
    ...
    methods: {
      loadCityList() {
        this.$http.jsonp('https://api.douban.com/v2/loc/list',{params:{count:48}})
        .then(function(res){
      ...
      loadData(cityId,day_type,type) {
        this.isOpen = true;
        this.$http.jsonp('https://api.douban.com/v2/event/list',{params:{count:60,loc:cityId,day_type:day_type,type:type}})
        .then(function(res){
          let data = res.body;
