  ...
  <div class="movie">
    ...
    <header>
      <select v-model="selected" @change="changeCity">
        <option class="option1" v-for="city in cityList" :value="city.id">{{city.name}}</option>
      </select>
...
<script>
  ...
  export default {
    ...
    mounted(){
      this.loadCityList();
      this.loadData(this.selecets,this.mType);
    },
    methods: {
      loadCityList() {
      ...
      loadData(cityId,type) {
        this.isOpen = true;
        this.$http.jsonp('https://api.douban.com/v2/movie/' 
          + type,{params:{
            city:cityId,
            count:100
          }}
